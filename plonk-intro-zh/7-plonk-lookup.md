...
如果当我们在 Plonk 电路中接入查表门，那么 Plonk 协议就要检查这个门的输入输出是否合法，然后就会去查我们事先预设的表格，看看其输入输出关系是否能在表中找到对应的一行。如果表中存在这样的条目，那么这个门就合法，否则被视为非法。
...
T &=(t_i, t\_{i+1}) & = & \{(1,2), (2,3), (3,4)\}\\
S^{lo} &= (s^{lo}\_i, s^{lo}\_{i+1}) & = & \{(1,2), (2,2), (2,3)\}\\
S^{hi} &= (s^{hi}\_i, s^{hi}\_{i+1}) & = & \{(3,4), (4,4), (4,4)\}\\
\end{split}
$$
